...
const getOrdersOfConnectedProductOwner = async (req, res) => {
    const serviceOwnerId = req.user._id;
    try {
        const serviceOwner = await ServiceOwner.findOne({ user: serviceOwnerId }).select('productOwner').exec()

        console.log(serviceOwner)
        const orders = await Order.find({
            $or: [
                {
                    targetedServiceOwners: serviceOwnerId,
                    'productOwners.productOwner': serviceOwner.productOwner.user,
                    'productOwners.status': 'Pending'
                },
                {
                    'productOwners.productOwner': serviceOwner.productOwner.user,
                    'productOwners.serviceOwner': serviceOwnerId,
        ...
        console.log(e)
        res.status(300).send('Error finding the orders')
    }
}

//change product status of the order
const changeStatusOfOrderToDelieverForProductOwner = async (req, res) => {
    const serviceOwnerId = req.user._id;
    ...
    try {
        ...
        order.productOwners.forEach((productOw) => {
            if (String(productOw.serviceOwner) === String(serviceOwnerId)) {
                productOw.status = status
                hisOrder = true
            }
            else if (productOw.status === 'Pending') {
                productOw.status = status
                productOw.serviceOwner = serviceOwnerId
                hisOrder = true
            }
        ...
        order.save()
        if (hisOrder) res.status(200).json("Order is Accpeted Successfully")
        else res.status(300).json("Order is Accpeted By Another Service Owner")
